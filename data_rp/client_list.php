...
$qry = '';
$qry=''; 
     $qry = "SELECT c.id, c.first_name, c.last_name, c.residance, c.gender, c.date_created, b.branch_name, g.group_name, c.data_id FROM clients c, branches b, groups g ";
  ...
  <div class="report_wrap">
        ...
        <div class="report_header" style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;padding:8px 0 8px 0;">
          ...
          <form name="form1" method="post" action="client_list.php" id="form1" style="display:flex;align-items:center;gap:12px;justify-content:flex-end;margin-bottom:8px;">
            ...
            <span style="background:#ddd;padding:5px 10px;border-radius:5px;margin-left:6px;font-size:12px;"><?php if($next<=$lastpage) echo "<a href='$target_page?page=$next&limit=$limit&month=$month&year=$year&gender=$gender&branch=$branch&address=$address'>Next</a>"; ?></span>
            <?php if($page!=1) { $prev = $page - 1; ?>
            <span style="background:#ddd;padding:5px 10px;border-radius:5px;margin-left:6px;font-size:12px;"><?php echo "<a href='$target_page?page=$prev&limit=$limit&month=$month&year=$year&gender=$gender&branch=$branch&address=$address'>Back</a>"; ?></span>
            <?php } ?>
            <span style="background:#ddd;padding:5px 10px;border-radius:5px;margin-left:6px;font-size:12px;"><?php echo "<a href='$target_page?page=1&limit=$total_pages&limit=$limit&month=$month&year=$year&gender=$gender&branch=$branch&address=$address'>View All</a>"; ?></span>
          </form>
       </div>
     </div>
  </div>
